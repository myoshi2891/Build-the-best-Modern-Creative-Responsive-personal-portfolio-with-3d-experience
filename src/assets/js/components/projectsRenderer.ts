...
export class ProjectsRenderer {
  ...
  private createProjectHTML(project: Project, totalProjects: number): string {
    const tagsHTML = project.tags.map(tag => `
      <span title="${escapeHTML(tag)}" class="coolFunnyLink">
    ...
    return `
      <div class="project">
        <div class="project__header">
          <span>${escapeHTML(project.id.toString())}/${totalProjects}</span>
          <span>${escapeHTML(project.date)}</span>
        </div>
        <div class="project__infos">
          <h1 class="project__infos--name">
            ${escapedTitle} <span>${escapedSubtitle}</span>
          </h1>
        </div>
  ...
  public render(): void {
    ...
    const projectsHTML = this.projectsList.map(project => 
      this.createProjectHTML(project, this.projectsList.length)
    ).join('');
